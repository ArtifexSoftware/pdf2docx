...
class RawPageFitz(RawPage):
    ...
    def extract_raw_dict(self, **settings):
        ...
        *_, w, h = self.page_engine.rect # always reflecting page rotation
        ...
        image_blocks = self._filter_decorative_shape_images(image_blocks, hyperlinks)
    ...
    def _filter_decorative_shape_images(images, hyperlinks):
        ...
        for image in images:
            ...
            if RawPageFitz._is_hyperlink_decorative_strip(bbox, hyperlink_rects):
                continue
    ...
    @staticmethod
    def _is_hyperlink_decorative_strip(bbox, hyperlink_rects):
        """Check whether a thin strip is likely a hyperlink underline decoration."""
        if bbox.is_empty:
            return False
        ...
        # horizontal underline-like strip
        if not (short <= 3.5 and long_ >= 8.0 and ratio >= 4.0 and w >= h * 2.0):
            return False

        covered_segments = []
        for rect in hyperlink_rects:
            link_h = rect.height or 1.0
            # near hyperlink bottom area
            baseline_gap = abs(bbox.y0 - rect.y1)
            if baseline_gap > max(1.5, 0.40 * link_h):
                continue
            if bbox.y0 < rect.y0 + 0.45 * link_h:
                continue

            x0 = max(bbox.x0, rect.x0)
            x1 = min(bbox.x1, rect.x1)
            if x1 > x0:
                covered_segments.append((x0, x1))

        if not covered_segments:
            return False

        # Merge overlapped segments and calculate total coverage on this strip.
        covered_segments.sort()
        merged = []
        for x0, x1 in covered_segments:
            if not merged or x0 > merged[-1][1]:
                merged.append([x0, x1])
            else:
                merged[-1][1] = max(merged[-1][1], x1)

        covered_w = sum(x1 - x0 for x0, x1 in merged)
        overlap_ratio = covered_w / (w or 1.0)
